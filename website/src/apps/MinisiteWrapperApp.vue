<template>
    <div class="minisite-wrapper-app">
        <div v-if="event && (event._original_data_id || event.__loading)" id="app">
            <WebsiteNav :minisite="event" :active-event-i-d="activeEventID" />
            ...
            <router-view class="my-3 slmngg-page" @id_change="(id) => activeEventID = id" />
        </div>
        <NotFoundPage v-else />
    </div>
</template>
