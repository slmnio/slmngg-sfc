...
export function isEventStaffOrHasRole(user, { event, role, websiteRoles } = {}) {
    ...
    if ([...(websiteRoles || []), role].some(websiteRole => (user.website_settings || []).includes(websiteRole))
        || (event.staff || []).map(x => dirtyID(x?.id) || dirtyID(x)).includes(dirtyID(user.airtableID))) return true;
