        ...
        <div class="container">
            <ContentRow v-if="team.accolades">
                <ContentThing :thing="accolade" type="event" :link-to="accolade.event" :theme="accolade.event && accolade.event.theme" v-for="accolade in team.accolades"
                              :key="accolade.id" :show-logo="true" :text="accolade.name" />
            </ContentRow>
            <ContentRow v-if="team.owners" :title="team.owners.length === 1 ? 'Owner' : 'Owners'">
                <ContentThing v-for="owner in team.owners" :key="owner.id"
            ...
            <ContentRow v-if="team.captains" :title="team.captains.length === 1 ? 'Captain' : 'Captains'">
                <ContentThing v-for="captain in team.captains" :key="captain.id"
                              type="player" :text="captain.name" :thing="captain" :theme="team.theme"></ContentThing>
            </ContentRow>
            <ContentRow v-if="team.staff" title="Team staff">
                <ContentThing type="player" :text="staff.name" :thing="staff" :theme="team.theme" v-for="staff in team.staff" :key="staff.id"></ContentThing>
            </ContentRow>
            <ContentRow v-if="team.players" title="Players">
                <ContentThing :show-headshot="team.show_headshots" type="player" :text="player.name" :thing="player" :theme="team.theme" v-for="player in team.players" :key="player.id"></ContentThing>
            </ContentRow>
            <ContentRow v-if="showLimitedPlayers" title="Players">
                <ContentThing type="player" :thing="player" no-link="true" :text="player.name" :theme="team.theme" v-for="player in team.limited_players" :key="player.name"></ContentThing>
            </ContentRow>
            <ContentRow v-if="team.sister_teams" :title="`Sister team${team.sister_teams.length === 1 ? '' : 's'}`">
                <ContentThing type="team" :show-logo="true" :text="item.name" :thing="item" :theme="item.theme" v-for="item in team.sister_teams" :key="item.id"></ContentThing>
            </ContentRow>
            <ContentRow v-if="team.team_in_other_tournaments" title="Team in other tournaments">
                <ContentThing type="team" :show-logo="true" :text="item.event ? `${item.name} (${item.event.short})` : item.name" :thing="item" :theme="item.theme" v-for="item in team.team_in_other_tournaments" :key="item.id"></ContentThing>
            </ContentRow>
