    ...
    <GenericOverlay
        ...
        <div v-for="(team, i) in teams" :key="team.id" class="team" :class="[`team-${i}`]">
            <ThemeTransition
                ...
                :end="(teams.length === 2) ? ((i === 0) ? 'right' : 'left') : 'bottom'"
                :start="(teams.length === 2) ? null : 'top'"
                :starting-delay="200"
                :inner-delay="200"
                :duration="600"
                :one-color="true">
...
export default {
    ...
    props: ["broadcast", "title", "showRoles", "sort", "animationActive", "showStaff", "splitPlayers", "showBadges", "virtualMatch", "gfx"],
    computed: {
        accentColor() {
        ...
        overridePlayers() {
            if (!this.gfx.markdown.includes("|")) return [];
            return this.gfx.markdown.split("#")[0].split("|").map(x => x.split(",").map((name) => {
                return {
                    id: name,
                    name: name
                };
        ...
        overrideTeamNames() {
            if (!this.gfx.markdown.includes("#")) return [];
            return this.gfx.markdown.split("#")[1].split("|");
        },
        teams() {
            if (this.gfx.teams) {
                if (this.gfx.teams.length === 1 && this.gfx.markdown.trim() !== "") {
                    const team = this.gfx.teams[0];
                    const [tOnePlayers, tTwoPlayers] = this.overridePlayers;
                    const [tOneName, tTwoName] = this.overrideTeamNames;
                    return [
                        { ...team, name: tOneName ? tOneName : team.name, showablePlayers: tOnePlayers},
                        { ...team, name: tTwoName ? tTwoName : team.name, showablePlayers: tTwoPlayers }
                    ];
                }
                return this.processTeams(this.gfx.teams);
            }
            if (!this.match) return [];
            return this.processTeams(this.match.teams);
        },
        highlight_event() {
